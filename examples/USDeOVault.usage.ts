/**
 * Example usage of USDeOVault and MultiCollateralToken
 *
...
// import { MultiCollateralToken__factory, USDeOVault__factory } from '../typechain-types'

/**
 * Example 1: User mints USDe with USDC
 */
async function exampleMintUSDe(
    usdeOVault: ethers.Contract,
    ...
    // 1. Approve USDeOVault to spend USDC
    console.log('1. Approving USDeOVault to spend USDC...')
    const approveTx = await usdcToken.connect(userSigner).approve(usdeOVault.address, usdcAmount)
    await approveTx.wait()
    console.log('   ✓ Approved')

    // 2. Mint USDe
    console.log('2. Minting USDe...')
    const mintTx = await usdeOVault.connect(userSigner).mintWithCollateral(usdcToken.address, usdcAmount)
...
async function exampleRedeemUSDe(
    ...
    const redeemTx = await usdeOVault.connect(userSigner).redeemForCollateral(usdcToken.address, usdeAmount)
 ...
 */
async function exampleEmergencyDisable(usdeOVault: ethers.Contract, gatekeeper: ethers.Signer) {
    console.log('Example: Emergency disable mint/redeem')

    console.log('1. Disabling mint and redeem...')
    const disableTx = await usdeOVault.connect(gatekeeper).disableMintRedeem()
    await disableTx.wait()
    console.log('   ✓ Mint and redeem disabled')

    // Verify
    const maxMint = await usdeOVault.maxMintPerBlock()
    const maxRedeem = await usdeOVault.maxRedeemPerBlock()
...
async function exampleERC4626Operations(
    ...
    const approveTx = await mct.connect(user).approve(usdeOVault.address, mctAmount)
    await approveTx.wait()

    // 2. Deposit MCT to get USDe
    console.log('2. Depositing MCT...')
    const userAddress = await user.getAddress()
    const depositTx = await usdeOVault.connect(user).deposit(mctAmount, userAddress)
    await depositTx.wait()
    console.log('   ✓ Deposited')

    // 3. Check shares received
    const shares = await usdeOVault.balanceOf(userAddress)
    ...
    const withdrawTx = await usdeOVault.connect(user).withdraw(mctAmount, userAddress, userAddress)
    await withdrawTx.wait()
    console.log('   ✓ Withdrawn')
}
 ...
 */
async function exampleQueryState(usdeOVault: ethers.Contract, mct: ethers.Contract) {
    console.log('Example: Querying contract state')

    // USDeOVault state
    console.log('\nUSDeOVault:')
    const maxMintPerBlock = await usdeOVault.maxMintPerBlock()
    const maxRedeemPerBlock = await usdeOVault.maxRedeemPerBlock()
    const mctAddress = await usdeOVault.mct()
    const totalSupply = await usdeOVault.totalSupply()
    const totalAssets = await usdeOVault.totalAssets()
