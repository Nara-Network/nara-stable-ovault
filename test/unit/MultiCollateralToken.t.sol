...
contract MultiCollateralTokenTest is TestHelper {
    ...
    function test_RedeemForUSDC() public {
        ...
        // Redeem (transfer MCT to USDe which has MINTER_ROLE to redeem)
        vm.startPrank(alice);
        mct.transfer(address(usde), mctAmount);
        vm.stopPrank();
    ...
    function test_RevertIf_InsufficientCollateral() public {
        ...
        vm.startPrank(address(usde));
        mct.mintWithoutCollateral(address(usde), 1000e18); // Extra unbacked MCT
    ...
    function testFuzz_MintRedeemRoundTrip(uint256 amount) public {
        ...
        // Transfer to usde for redemption
        vm.prank(alice);
        mct.transfer(address(usde), mctAmount);
